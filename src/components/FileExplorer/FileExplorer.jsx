      ...
      <div key={item.id} className="list-item-container">
        <div className="list-item">
          <div className="label-container">
            {item.isFolder && (
              <span onClick={() => handleExpand(item.id)}>
                {isExpanded[item.id] ? (
            ...
            {item.isFolder ? (
              ...
              >
                <g transform="translate(1.41 1.41) scale(2.81)">
                  <path
                    fill="rgb(224,173,49)"
                    d="M0 68.798v11.914c0 1.713 1.401 3.114 3.114 3.114 3.344 0 4.805-2.642 4.805-2.642L8.14 29.281l2.739-2.827 72.894-2.977v-1.482c0-2.396-1.942-4.338-4.338-4.338H50.236c-1.15 0-2.254-.457-3.067-1.27l-8.943-8.943c-.813-.813-1.917-1.27-3.067-1.27H4.338C1.942 6.174 0 8.116 0 10.512v7.146v2.332v48.808z"
                  />
                  <path
                    fill="rgb(255,200,67)"
                    d="M3.114 83.826c1.713 0 3.114-1.401 3.114-3.114V27.81c0-2.393 1.94-4.333 4.333-4.333h75.107c2.393 0 4.333 1.94 4.333 4.333v51.684c0 2.393-1.94 4.333-4.333 4.333H3.114z"
                  />
                </g>
              </svg>
            ) : (
              ...
              >
                <g transform="translate(1.41 1.41) scale(2.81)">
                  <path
                    ...
                    d="M78.42 18.345v68.502c0 1.741-1.412 3.153-3.153 3.153H14.733c-1.741 0-3.153-1.412-3.153-3.153V3.153C11.58 1.412 12.991 0 14.733 0h45.343C63.133 7.61 69.386 13.658 78.42 18.345z"
                  />
                  <path
                    fill="rgb(217,215,202)"
                    d="M78.42 18.345H62.948c-1.587 0-2.873-1.286-2.873-2.873V0L78.42 18.345z"
                  />
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && e.target.value != '') {
                      handleFilenameChange(e.target.value.trim(), item.id);
                    }
                  }}
                />
                <button
                  ...
                  onClick={() => {
                    ...
                    handleFilenameChange(labelValue.trim(), item.id);
                    setLabelValue("");
                  }}
                >
            ...
            {item.isFolder && (
              <>
                <button
                  ...
                  onClick={() => {
                    handleExpand(item.id, true);
                    handleAddNew({ isFolder: true, parentId: item.id });
                  }}
                >
                  <FontAwesomeIcon icon={faFolderPlus} color="#929292ff" />
                </button>
                <button
                  className="button add-icon"
                  onClick={() => {
                    handleExpand(item.id, true);
                    handleAddNew({ isFolder: false, parentId: item.id });
                  }}
                >
                  <FontAwesomeIcon icon={faFileCirclePlus} color="#929292ff" />
                </button>
              </>
            )}
            <button
            ...
            >
              <FontAwesomeIcon icon={faTrash} color="#929292ff" />
            </button>
          </div>
          {item.isFolder &&
            item.children &&
            isExpanded[item.id] &&
            render({ dataList: item.children })}
        </div>
      </div>
    ));
  }

  return render({ dataList: data });
};
...
const FileExplorer = () => {
  ...
  function handleExpand(id, value = false) {
    setIsExpanded((prev) => ({ ...prev, [id]: value || !prev[id] }));
  }

  function handleAddNew({ isFolder, parentId }) {
    setData((prev) => {
      function findParent({ fileData = [] }) {
        fileData.forEach((item) => {
          if (item.id === parentId) {
            ...
            item.children = [...(item.children || []), _tempChild];
            return;
          } else {
            findParent({ fileData: item.children });
          }
        });
      ...
      return findParent({ fileData: structuredClone(prev) });
    });
  }

  function handleDelete(id) {
    setData((prev) => {
      const isDeleted = prev.find((item) => item.id === id);
      if (isDeleted) {
        return prev.filter((item) => item.id !== id);
      } else {
        function deleteItem({ fileData }) {
          fileData.forEach((item) => {
            if (!item.children) return;
            const isDeleted = item.children.find((child) => child.id === id);
            if (isDeleted) {
              console.log(isDeleted);
              item.children = item.children.filter((child) => child.id !== id);
              return;
            } else {
              deleteItem({ fileData: item.children });
            }
          });
        ...
        return deleteItem({ fileData: structuredClone(prev) });
      }
    });
  }

  function handleFilenameChange(label, id) {
    setData((prev) => {
      function findParent({ fileData = [] }) {
        fileData.forEach((item) => {
          if (item.id === id) {
